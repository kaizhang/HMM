...
kmeans :: (G.Vector v1 Double, G.Vector v2 Int, Eq (v2 Int), PrimMonad m)
       ...
       -> m (v2 Int, M.Matrix v1 Double)          -- ^ membership vector
kmeans g k mat = do
...
kmeansWith :: (G.Vector v1 Double, G.Vector v2 Int, Eq (v2 Int))
           ...
           -> (v2 Int, M.Matrix v1 Double) -- ^ membership vector and centroid
...
forgyMethod :: (PrimMonad m, G.Vector v a, Eq a) => Gen (PrimState m) -> Int -> M.Matrix v a -> m (M.Matrix v a)
forgyMethod g k mat | k > n = error "k is larger than sample size"
                    | otherwise = iter
  where
    iter = do
        vec <- sample g k . U.enumFromN 0 $ n
        let xs = map (G.toList . M.takeRow mat) . G.toList $ vec
        if length (nub xs) == length xs
           then return . M.fromLists $ xs
           else iter
    n = M.rows mat
